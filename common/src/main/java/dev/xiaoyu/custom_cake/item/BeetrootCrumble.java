...
public class BeetrootCrumble extends Item {
    ...
    public @NotNull InteractionResult useOn(UseOnContext context) {
        ...
        if (blockState.getBlock() == CustomCake.BASE_CAKE_BLOCK.get() && blockState.getValue(CakeBlock.BITES) == 0) {
            level.setBlockAndUpdate(pos, CustomCake.BEETROOT_CAKE_BLOCK.get().defaultBlockState());

            if (level.getBlockEntity(pos) instanceof BaseCakeBlockEntity cakeEntity)
                cakeEntity.getPersistentData().putBoolean("created_by_beetroot_crumble", true);
